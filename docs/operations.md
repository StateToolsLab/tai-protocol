...
保存するのは発行済みの原文であり、プレースホルダーや草稿ではない。
...
Architect側に、後任からも取得可能なReport原文が既に保存されているなら、
リポジトリにReportを二重保存する必要はない。状態文書にその固定参照・保持条件を残す。
...
Supervisorが発行後に補完せず、不足時はArchitectへ差し戻す。
...
既存ブリッジのallowlistにはPython・grep・wc・sha256sumは含まれない。
このリリースはそれらを自動追加しない。必要な能力は発行前に検査し、権限不足なら
`blocked` Reportか対話実行へ縮退する。応答者のいないheadlessで質問待ちしない。
